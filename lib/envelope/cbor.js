...
const cborValue = P.mappedTag(P.bits(3), {
    uint: [0, cborUint],
    negint: [1, cborNegint],
    bytes: [2, P.lazy(() => cborLength(P.bytes, cborValue))],
    string: [3, P.lazy(() => cborLength(P.string, cborValue))],
    array: [4, cborArrLength(P.lazy(() => cborValue))],
    map: [5, P.lazy(() => cborArrLength(P.tuple([cborValue, cborValue])))],
    tag: [6, P.tuple([cborUint, P.lazy(() => cborValue)])],
...
exports.CBOR = P.apply(cborValue, {
    encode(from) {
        ...
        if (from.TAG === "map" && typeof value === "object" && value !== null) {
            return Object.fromEntries(from.data.map(([k, v]) => [
                this.encode(k),
                this.encode(v),
            ]));
        }
